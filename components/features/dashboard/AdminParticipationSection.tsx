                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <div className="flex flex-col items-center justify-center gap-4">
                        ...
                        <div className="flex flex-wrap justify-center gap-3">
                            {[1, 2, 3, 4].map(i => (
                                <Skeleton key={i} className="h-3 w-28 rounded-full" />
                            ))}
                        </div>
                    </div>
                    {/* Stats skeleton */}
...
export default function AdminParticipationSection() {
    ...
    const load = async () => {
        setLoading(true);
        setError(false);
        const res = await fetchParticipation();
        if (res) setData(res);
        else setError(true);
        setLoading(false);
    };
    ...
    if (loading) return <ParticipationSkeleton />;
    if (error || !data) return <ParticipationError onRetry={load} />;
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <ResponsiveContainer width="100%" height={380}>
                                <PieChart>
                                    <Pie
                                        data={pieData}
                                        cx="50%" cy="50%"
                                        innerRadius={110} outerRadius={155}
                                        paddingAngle={3}
                                        dataKey="value"
                                        isAnimationActive
                                        animationBegin={100}
                    ...
                    <div className="flex flex-col gap-3 self-center">
                        {stats.map(({ label, value, sub, icon: Icon, color }, i) => (
                            <div
                                key={label}
                                className="flex items-center gap-3 p-3 rounded-xl border bg-muted/30 animate-in fade-in slide-in-from-right-4 duration-500"
                    ...
                    <div className="mt-6 pt-5 border-t border-muted animate-in fade-in slide-in-from-bottom-2 duration-500 delay-300">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
                            {(data.by_department ?? []).map(({ name, rate, active, total }, i) => (
                                <div
                                    key={name ?? i}
                                    className="flex flex-col gap-2 p-3 rounded-xl border bg-muted/30 animate-in fade-in zoom-in-95 duration-300"
